...
#include <sys/socket.h> // provides functions to establish socket connection, accept clients, bind and listen.
...
typedef struct
{
    ...
    size_t read_pos;               // used to track current posistion where we are writing/reading in the above buffer
    int active;                    // 1 or 0: if the value is 0, then array/buffer is empty and this fd can be reused for a new user
} client_t;
...
typedef struct
{
    ...
    // so select() is a thread that:
    // watches clients aka fd aka sockets
    // it goes to sleep (which means no CPU usage while listening for data change)
    // kernel wakes it up whenever a data is available at any of the given sockets
    // select() returns that this particular socket(s) is ready and we read data from it
